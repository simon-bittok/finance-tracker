...
app.get("/history", requireAuth, async (c) => {
	...
	if (!from || !to) {
		const historicalTransactions =
			await transactionRepository.groupWeeklyTransactionsByType(userId, prisma);

		return c.json(historicalTransactions);
	}
	...
	if (fromDate.getDate() > toDate.getDate()) {
		return c.json({ error: "To date must be later than From date" }, 422);
	}

	const historicalData =
		await transactionRepository.groupTransactionsPerodicallyByType(
			userId,
